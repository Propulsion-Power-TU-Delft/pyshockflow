...
for i,res in enumerate(sols):
    ax[0].plot(res['X Coords'], res['Primitive']['Density'][:, -1], label=labels[i], ls=lss[i]) 
ax[0].set_ylabel(r'$\rho \ \rm{[kg/m^3]}$')
# Inset for Plot 1
inset1 = inset_axes(ax[0], width="30%", height="30%", loc='upper right')
inset1.plot(rhoRef[:,0], rhoRef[:,1], 'ko', mfc='none', ms=ms_small)
for i, res in enumerate(sols):
    inset1.plot(res['X Coords'], res['Primitive']['Density'][:, -1], ls=lss[i])
    inset1.grid(alpha=.3)
...
for i,res in enumerate(sols):
    ax[1].plot(res['X Coords'], res['Primitive']['Velocity'][:, -1], ls=lss[i]) 
ax[1].set_ylabel(r'$u \ \rm{[m/s]}$')
# Inset for Plot 3
# inset3 = inset_axes(ax[1], width="35%", height="35%", loc='upper left')
inset3 = ax[1].inset_axes([0.225, 0.625, 0.3, 0.3])
inset3.plot(uRef[:,0], uRef[:,1], 'ko', mfc='none', ms=ms_small)
for i, res in enumerate(sols):
    inset3.plot(res['X Coords'], res['Primitive']['Velocity'][:, -1], ls=lss[i])
    inset3.grid(alpha=.3)
...
for i,res in enumerate(sols):
    ax[2].plot(res['X Coords'], res['Primitive']['Pressure'][:, -1]/1e6, ls=lss[i]) 
ax[2].set_ylabel(r'$p \ \rm{[MPa]}$')
# Inset for Plot 2
inset2 = inset_axes(ax[2], width="30%", height="30%", loc='upper right')
inset2.plot(pRef[:,0], pRef[:,1], 'ko', mfc='none', ms=ms_small)
for i, res in enumerate(sols):
    inset2.plot(res['X Coords'], res['Primitive']['Pressure'][:, -1]/1e6, ls=lss[i])
    inset2.grid(alpha=.3)
