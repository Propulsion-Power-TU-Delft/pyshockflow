...
set_thesis_style()
fig, ax = create_figure(fraction=1.0, aspect_ratio=0.9, subplots=(1, 3), is_print=False)

# REFERENCE RESULTS
with open('reference.pik', 'rb') as file:
    data = pickle.load(file)
mach = data.u[:,-1]/(data.p[:,-1]*1.4/data.rho[:,-1])**0.5
ax[0].plot(data.x+0.5, data.rho[:,-1], '-k', label='Reference')
ax[1].plot(data.x+0.5, data.u[:,-1], '-k', label='Reference')
ax[2].plot(data.x+0.5, data.p[:,-1], '-k', label='Reference')
...
for i,results in enumerate(resultsPickle):
    ax[0].plot(
        ...
        label=labels[i+1], color=colors[i], ls=ls[i])
...
inset0.plot(data.x+0.5, data.rho[:,-1], '-k')
for i, results in enumerate(resultsPickle):
    inset0.plot(results['X Coords'][1:-1], results['Primitive']['Density'][1:-1, -1], color=colors[i], ls=ls[i])
...
inset1 = inset_axes(ax[1], width="40%", height="40%",
                    ...
                    loc='lower left')
inset1.plot(data.x+0.5, data.u[:,-1], '-k')
for i, results in enumerate(resultsPickle):
    inset1.plot(results['X Coords'][1:-1], results['Primitive']['Velocity'][1:-1, -1], color=colors[i], ls=ls[i])
...
inset2.plot(data.x+0.5, data.p[:,-1], '-k')
for i, results in enumerate(resultsPickle):
    inset2.plot(results['X Coords'][1:-1], results['Primitive']['Pressure'][1:-1, -1], color=colors[i], ls=ls[i])
...
fig.legend(labels, loc='outside upper center', ncol=len(labels)//2)
...
plt.savefig(outFolder + '/flux_limiters_100.pdf')
